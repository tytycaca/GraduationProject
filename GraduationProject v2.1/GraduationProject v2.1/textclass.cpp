	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, (char*)"../GraduationProject v2.1/data/fontdata.txt", (WCHAR*)L"../GraduationProject v2.1/data/font.dds");
	if (!result)
	{
...
bool TextClass::SetDisplayTestVersion(ID3D11DeviceContext* deviceContext)
{
	char displayString[32];
	bool result;

	strcpy_s(displayString, "THIS IS TEST VERSION");

	result = UpdateSentence(m_sentence[7], displayString, 20, 180, 1.0f, 0.0f, 0.0f, deviceContext);
	if (!result) return false;
